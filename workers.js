...
export default {
    async fetch(request, env, ctx) {
        ...
        // 但如果带有 token、dev 或 zerotrust 参数，则允许访问后台页面
        if (path === '/admin.html') {
            const token = url.searchParams.get('token');
            const dev = url.searchParams.get('dev');
            const zerotrust = url.searchParams.get('zerotrust');
            if (token || dev || zerotrust) {
                // 有 token、dev 或 zerotrust 参数，允许访问后台
...
function createErrorResponse(message, status = 500, headers = {}) {
...
function getAuthConfig(request, env, url) {
    ...
    // 获取认证方式
    const authMethod = env.AUTH_FUNC;
    ...
    // AUTH_FUNC 未配置或无效时，返回错误信息
    if (!authMethod || (authMethod !== 'dev' && authMethod !== 'key' && authMethod !== 'zerotrust')) {
        authConfig.error = '服务器认证方式配置无效，请联系管理员（AUTH_FUNC 必须为 dev、key 或 zerotrust）';
    } else if (authMethod === 'key') {
        // key 模式：检查 AUTH_KEY 是否配置
        if (!env.AUTH_KEY || env.AUTH_KEY.length === 0) {
            authConfig.error = '服务器未配置 AUTH_KEY，请联系管理员';
        } else if (isDev) {
            // DEV 环境返回 token 用于自动填充（可选）
            authConfig.hasAuthKey = true;
        } else {
            authConfig.hasAuthKey = true;
        }
    } else if (authMethod === 'dev') {
        // dev 模式：直接进入
        authConfig.isQuickAccess = true;
    } else if (authMethod === 'zerotrust') {
        // zerotrust 模式：cloudflare 已认证，直接进入
        authConfig.isQuickAccess = true;
    }
    ...
    try {
        ...
        if (authMethod === 'zerotrust') {
            ...
            if (env.AUTH_KEY) {
                ...
                return createErrorResponse('服务器未配置 AUTH_KEY，请联系管理员', 500);
            }
        } else if (authMethod === 'dev') {
            // DEV 模式：直接通过认证
            isValid = true;
        } else {
            return createErrorResponse('认证方式配置无效', 500);
        }
...
async function handleAPI(request, env, url) {
    ...
    try {
        ...
        // POST /api/update-secret - 更新 Secret（开发环境仅返回模拟成功）
        if (method === 'POST' && path === '/api/update-secret') {
            return await updateSecret(request, env);
        }

        return new Response(JSON.stringify({
            success: false,
            error: 'API not found',
            available: ['GET /api/config', 'GET /api/config/:group', 'POST /api/update', 'POST /api/reset', 'POST /api/update-secret']
        }), {
            status: 404,
...
async function getConfig(env) {
    try {
        ...
        if (!env.PROFILE_DB) {
            ...
            return createErrorResponse('Database binding not found', 500);
        }
        ...
        for (const group of groups) {
            ...
            config[group] = result?.config_data ? JSON.parse(result.config_data) : {};
        }
...
async function getConfigGroup(env, group) {
    ...
    if (!result) {
        return createErrorResponse('配置组不存在', 404);
    }
...
async function updateConfig(request, env) {
    try {
        ...
        if (!group || data === undefined) {
            return createErrorResponse('缺少必要参数：group, data', 400);
        }

        // 验证配置组
        const validGroups = ['profile', 'socialLinks', 'appearance', 'effects', 'security'];
        if (!validGroups.includes(group)) {
            return createErrorResponse('无效的配置组', 400);
        }
        ...
        if (version !== undefined && current && version !== current.version) {
            return createErrorResponse('版本冲突，配置已被修改', 409, {
                'X-Current-Version': current.version.toString()
            });
        }
        ...
        if (verify && verify.config_data === jsonData) {
            ...
            return createErrorResponse('写入后验证失败', 500);
        }
    } catch (error) {
        return createErrorResponse(error.message, 500);
    }
...
// 更新 Secret
async function updateSecret(request, env) {
    try {
        // 鉴权检查
        const authResult = await verifyAuth(request, env);
        if (!authResult.valid) {
        ...
        if (!secretValue) {
            return createErrorResponse('缺少必要参数：secretValue', 400);
        }

        if (secretValue.length < 6) {
            return createErrorResponse('AUTH_KEY 长度至少为 6 位', 400);
        }
        ...
        const serviceName = env.CLOUDFLARE_WORKER_NAME || env.CF_WORKER_NAME || env.WORKER_NAME || env.CF_SERVICE_NAME || env.__WORKER_NAME || 'profile-page-prod';

        if (!accountId || !apiToken) {
            return createErrorResponse('服务器未配置 Cloudflare API 凭据', 500);
        }

        const response = await fetch(
            `https://api.cloudflare.com/client/v4/accounts/${accountId}/workers/scripts/${serviceName}/secrets`,
            {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
        ...
        if (!response.ok || result.success === false) {
            return createErrorResponse(result.errors?.[0]?.message || 'Cloudflare Secret 更新失败', response.status || 500);
        }
...
// 缓存管理（KV + Cache API）
// ============================================
// KV 缓存：用于配置数据（通过 CONFIG_CACHE 绑定）
// Cache API：用于静态资源（通过 caches.open() 访问）
...
// 写入 Cache API
async function writeToCache(url, response, ttl = 3600) {
    try {
        const cache = caches.default;
        // 复制响应以便缓存
...
async function verifyAuth(request, env) {
    // AUTH_FUNC 未配置时，拒绝访问
    if (!env.AUTH_FUNC) {
        return { valid: false, error: '服务器未配置认证方式，请联系管理员' };
    }
    ...
    // 允许开发环境的特殊 token 绕过认证
    const devAuthHeader = request.headers.get('Authorization');
    if (devAuthHeader && (devAuthHeader.startsWith('Bearer __dev__') || devAuthHeader.startsWith('Bearer __zerotrust__'))) {
        return { valid: true, user: 'dev-bypass' };
    }
    ...
    if (!hasAuthKey && !hasZeroTrust) {
        return { valid: false, error: '服务器认证配置不完整，请联系管理员' };
    }
